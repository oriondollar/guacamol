...
class ValidityBenchmark(DistributionLearningBenchmark):
    ...
    def __init__(self, number_samples, return_valid=True, use_filters=False) -> None:
    ...
    def assess_model(self, model, prior_gen=[]):
        ...
        n_gen = self.number_samples - len(prior_gen)
        molecules = model.generate(number_samples=n_gen)
        molecules += prior_gen
        end_time = time.time()
...
class UniquenessBenchmark(DistributionLearningBenchmark):
    ...
    def __init__(self, number_samples, return_unique=True, use_filters=False) -> None:
    ...
    def assess_model(self, model, prior_gen=[]):
        ...
        molecules = sample_valid_molecules(model=model, number_molecules=self.number_samples,
                                           prior_gen=prior_gen, use_filters=self.use_filters)
        end_time = time.time()
        ...
        result =  DistributionLearningBenchmarkResult(benchmark_name=self.name,
                                                      ...
                                                      sampling_time=end_time - start_time,
                                                      metadata=metadata)
        if self.return_unique:
            return result, unique_molecules,
        else:
            return result
...
class KLDivBenchmark(DistributionLearningBenchmark):
    ...
    def assess_model(self, model, prior_gen=[]):
        ...
        if len(prior_gen) > 0:
            ...
            molecules = sample_unique_molecules(model=model, number_molecules=n_gen, max_tries=2)
        end_time = time.time()
...
class FragBenchmark(DistributionLearningBenchmark):
    ...
    def assess_model(self, model, prior_gen=[]):
        ...
        result = DistributionLearningBenchmarkResult(benchmark_name=self.name+'_{}'.format(self.type),
                                                     score=score,
                                                     sampling_time=end_time-start_time,
                                                     metadata=metadata)
        return result
